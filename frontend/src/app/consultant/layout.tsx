    ...
    <ShellFrame
        ...
        <div className="flex items-center justify-between gap-3 px-4 py-3">
          <div className="flex min-w-0 items-center gap-3">
            ...
            <div className="min-w-0">
              ...
              <p className="text-[11px] text-gray-500">Reporting Module</p>
            </div>
          </div>
          <button
            type="button"
      ...
      sidebar={({ close }) => (
        <>
          <div className="border-b border-gray-100 px-5 py-5">
            <div className="flex items-center justify-between gap-3">
              <div className="min-w-0">
                ...
                <p className="mt-1 text-xs text-gray-400">Reporting Module</p>
              </div>
              <button
                type="button"
                onClick={close}
          ...
          <nav className="mobile-scroll flex-1 space-y-1 overflow-y-auto px-2 py-4">
            {NAV.map((l) => {
              ...
              return (
                <Link
                  ...
                  href={l.href}
                  onClick={close}
                  className={`flex items-center gap-3 rounded-xl px-3 py-3 text-sm font-medium transition-colors ${active ? 'bg-primary-50 text-primary-700 shadow-sm' : 'text-gray-500 hover:bg-gray-50 hover:text-gray-800'}`}
                >
                  <span>{l.icon}</span>
                  {l.label}
                </Link>
            ...
            {(user?.role === 'manager' || user?.role === 'platform_admin') && (
              <Link
                href="/manager"
                onClick={close}
                className={`flex items-center gap-3 rounded-xl px-3 py-3 text-sm font-medium transition-colors ${pathname.startsWith('/manager') ? 'bg-primary-50 text-primary-700 shadow-sm' : 'text-gray-500 hover:bg-gray-50 hover:text-gray-800'}`}
              >
                <span>👔</span>
                Manager View
              </Link>
